...
import com.pt.ifp.neolauncher.adapter.PickIconAdapter;
import com.pt.ifp.neolauncher.app.NeoLauncherApp;
import com.pt.ifp.neolauncher.graphics.BackgroundBlur;
import com.pt.ifp.neolauncher.graphics.IconPack;
...
import com.pt.ifp.neolauncher.widget.Dialog;
import com.pt.ifp.neolauncher.widget.OptionsDialog;
...
public class PickIconActivity extends Activity {
	...
	public static void askToHide(Context context, ComponentName componentName,
		...
		Dialog.newDialog(context)
				...
				.setPositiveButton(android.R.string.ok, (d, w) -> {
					NeoLauncherApp.appMenu.hiddenApps.addAndStore(context,
							componentName);
					NeoLauncherApp.appMenu.updateIconsAsync(context);
	...
	protected void onCreate(Bundle state) {
		...
		BackgroundBlur.blurIfTrue(getWindow(),
				NeoLauncherApp.getPrefs(this).getBackgroundBlurRadius());
		kb = new SoftKeyboard(this);
		toolbarBackground = new ToolbarBackground(getResources());
		View toolbar = findViewById(R.id.toolbar);
		progressView = findViewById(R.id.progress);

		iconPackPackageName = NeoLauncherApp.iconPack
				.getSelectedIconPackageName();
		if (iconPackPackageName == null) {
			IconPack.Pack pack =
					NeoLauncherApp.iconPack.packs.values().iterator().next();
			if (pack != null) {
	...
	private void initGridView(ComponentName componentName) {
		...
		gridView.setOnItemClickListener((parent, view, position, id) -> {
			NeoLauncherApp.iconPack.addMapping(
					iconPackPackageName,
					componentName,
					iconAdapter.getItem(position));
			NeoLauncherApp.iconPack.storeMappings(this);
			NeoLauncherApp.appMenu.updateIconsAsync(this);
			finish();
		});
	...
	private void initReset(ComponentName componentName) {
		...
		if (NeoLauncherApp.iconPack.hasMapping(componentName)) {
	...
	private void askToRestore(ComponentName componentName) {
		new AlertDialog.Builder(this)
				...
				.setPositiveButton(android.R.string.ok, (d, w) -> {
					NeoLauncherApp.iconPack.removeMapping(componentName);
					NeoLauncherApp.iconPack.storeMappings(this);
					NeoLauncherApp.appMenu.updateIconsAsync(this);
					finish();
				})
				.setNeutralButton(R.string.all, (d, w) -> {
					NeoLauncherApp.iconPack.clearMappings();
					NeoLauncherApp.iconPack.storeMappings(this);
					NeoLauncherApp.appMenu.updateIconsAsync(this);
					finish();
				})
	...
	private void initHide(ComponentName componentName) {
		...
		if (NeoLauncherApp.appMenu.isDrawerPackageName(
	...
	private void initSwitchPack() {
		...
		HashMap<String, String> map = NeoLauncherApp.iconPack.getIconPacks();
		if (!map.isEmpty()) {
	...
	private void loadPack(String packageName) {
		...
		Executors.newSingleThreadExecutor().execute(() -> {
			IconPack.Pack pack = NeoLauncherApp.iconPack.packs.get(
					packageName);
			if (pack != null) {
